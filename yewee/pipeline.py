        ...
        try:
            while not self._stop.is_set():
                ...
                if self.osc is not None:
                    ...
                    # coordinates are ready now, and a receiver drawing its
                    # own graphics should not wait on rendering ours.
                    t0 = time.perf_counter()
                    try:
                        self.osc.send(tracks, frame.shape[:2], fps_ema)
                    except Exception as exc:   # same rule as every other feed
